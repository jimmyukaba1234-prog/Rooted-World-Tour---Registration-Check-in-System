...
# ==================== CHECK-IN PAGE ====================
    ...
    with tab_manual:
        ...
        # ─────────────────────────────────────
        # 👤 ADD NAME SEARCH CHECK-IN OPTION
        # ─────────────────────────────────────
        search_method = st.radio(
            "Search by:",
            ["Ticket ID", "Name Search"],
            horizontal=True,
            key="search_method"
        )
        
        if search_method == "Ticket ID":
            manual_ticket = st.text_input(
                "Enter Ticket ID:",
                placeholder="RWT-ABC123DEF",
                key="manual_ticket"
                ...
                if conn:
                    ...
                    if result:
                        ...
                        if status == 'checked_in':
                            st.warning(f"⚠️ {full_name} already checked in")
                        else:
                            st.info(f"**Attendee:** {full_name}")
                            ...
                            with col_btn2:
                                ...
                                if st.button("👤 View Details", type="secondary", use_container_width=True):
                                    st.session_state.view_ticket = manual_ticket
                    else:
            ...
            with col_name1:
                search_first = st.text_input(
                    "First Name:",
                    placeholder="Enter first name",
                    key="search_first"
                )
            with col_name2:
                search_last = st.text_input(
                    "Last Name:",
            ...
            # Search button
            if st.button("🔍 Search Attendee", type="primary", use_container_width=True):
                if not search_first and not search_last:
                    st.warning("Please enter at least a first name or last name")
                else:
                    conn = st.session_state.db.get_connection()
                    if conn:
                        # Build search query
                        if search_first and search_last:
                            query = "SELECT * FROM registrations WHERE first_name LIKE ? AND last_name LIKE ?"
                            params = [f"%{search_first}%", f"%{search_last}%"]
                        elif search_first:
                            query = "SELECT * FROM registrations WHERE first_name LIKE ?"
                            params = [f"%{search_first}%"]
                        else:
                            query = "SELECT * FROM registrations WHERE last_name LIKE ?"
                            params = [f"%{search_last}%"]
                        ...
                        if results:
                            ...
                            for idx, result in enumerate(results):
                                # Extract data based on your database schema
                                ticket_id = result[0]  # Adjust index based on your DB
                                first_name = result[1]
                                last_name = result[2]
                                email = result[3]
                                status = result[4] if len(result) > 4 else 'registered'
                                ...
                                with st.container():
                                    ...
                                    with col_result1:
                                        ...
                                        # Status badge
                                        if status == 'checked_in':
                                            st.markdown('<span class="status-badge status-checked_in">✅ Checked In</span>', 
                                                    unsafe_allow_html=True)
                                        else:
                                            st.markdown('<span class="status-badge status-registered">📝 Registered</span>', 
                                                    unsafe_allow_html=True)
                                    
                                    with col_result2:
                                        if status != 'checked_in':
                                            if st.button(f"Check-in", 
                                                        key=f"checkin_{ticket_id}_{idx}",
                                                        type="primary",
                                                        use_container_width=True):
                                                with st.spinner(f"Checking in {first_name}..."):
                                                    success, attendee = st.session_state.db.quick_checkin(ticket_id)
                                                    if success:
                                                        st.success(f"✅ Welcome {attendee[0]} {attendee[1]}!")
                                                        st.balloons()
                            ...
                            st.warning("No attendees found with that name.")
                    else:
                        # Demo mode - simulate search results
                        demo_attendees = [
                            {
                                'ticket_id': 'RWT-JOHNDOE123',
                                'first_name': 'John',
                                'last_name': 'Doe',
                        ...
                        # Filter demo results based on search
                        filtered_results = []
                        for attendee in demo_attendees:
                            if (search_first and search_first.lower() in attendee['first_name'].lower()) or \
                            (search_last and search_last.lower() in attendee['last_name'].lower()):
                                filtered_results.append(attendee)
                        
                        if filtered_results:
                            st.success(f"Found {len(filtered_results)} matching attendee(s) (Demo Mode)")
                            ...
                            for idx, result in enumerate(filtered_results):
                                with st.container():
                                    ...
                                    with col_result1:
                                        st.markdown(f"**👤 {result['first_name']} {result['last_name']}**")
                                    ...
                                    with col_result2:
                                        if result['status'] != 'checked_in':
                                            if st.button(f"Check-in", 
                                                        key=f"demo_checkin_{result['ticket_id']}",
                                                        type="primary",
                                                        use_container_width=True):
                                                st.success(f"✅ Demo check-in for {result['first_name']} {result['last_name']}!")
                                                st.balloons()
                                        else:
                                            st.info("✅ Checked in")
                        else:
            ...
            # ─────────────────────────────────────
            # 📄 VIEW ALL ATTENDEES (for quick access)
            # ─────────────────────────────────────
            st.markdown("---")
            with st.expander("👥 View All Attendees"):
                conn = st.session_state.db.get_connection()
                if conn:
                    cursor = conn.cursor()
                    cursor.execute("""
                    ...
                    if all_attendees:
                        ...
                        search_all = st.text_input("🔍 Filter list:", placeholder="Search by name or email")
                        
                        for attendee in all_attendees:
                            ticket_id, first_name, last_name, email, status = attendee
                            full_name = f"{first_name} {last_name}"
                            
                            # Filter if search term is provided
                            if search_all and search_all.lower() not in full_name.lower() and search_all.lower() not in email.lower():
                                continue
                            
                            col_attendee1, col_attendee2, col_attendee3 = st.columns([3, 1, 1])
                            
                            with col_attendee1:
                                st.markdown(f"**{full_name}**")
                                st.caption(f"Email: {email} | Ticket: {ticket_id}")
                            ...
                            with col_attendee3:
                                if status != 'checked_in':
                                    if st.button("Check-in", key=f"quick_{ticket_id}"):
                                        with st.spinner(f"Checking in {first_name}..."):
                                            success, attendee_info = st.session_state.db.quick_checkin(ticket_id)
                                            if success:
                                                st.success(f"✅ {first_name} checked in!")
                                                st.rerun()
                                else:
                                    st.info("✅ Done")
                    else:
                    ...
                    st.info("Database not connected - running in demo mode")
        
        # ─────────────────────────────────────
        # 🎫 VIEW TICKET DETAILS (if selected)
        # ─────────────────────────────────────
        if 'view_ticket' in st.session_state and st.session_state.view_ticket:
            st.markdown("---")
            ...
            if conn:
                ...
                cursor.execute("""
                    SELECT ticket_id, first_name, last_name, email, phone, 
                        emergency_contact, medical_notes, worship_team, 
                        volunteer, status, registration_time, checkin_time
                    FROM registrations 
                    WHERE ticket_id = ?
                """, (st.session_state.view_ticket,))
                
                result = cursor.fetchone()
                conn.close()
                
                if result:
                    cols = ['ticket_id', 'first_name', 'last_name', 'email', 'phone', 
                        'emergency_contact', 'medical_notes', 'worship_team', 
                        'volunteer', 'status', 'registration_time', 'checkin_time']
                    
                    attendee_data = dict(zip(cols, result))
                    ...
                    with col_detail1:
                        ...
                        st.markdown(f"**Name:** {attendee_data['first_name']} {attendee_data['last_name']}")
                        st.markdown(f"**Email:** {attendee_data['email']}")
                        if attendee_data['phone']:
                            st.markdown(f"**Phone:** {attendee_data['phone']}")
                        if attendee_data['emergency_contact']:
                            st.markdown(f"**Emergency Contact:** {attendee_data['emergency_contact']}")
                        if attendee_data['medical_notes']:
                            st.markdown(f"**Medical Notes:** {attendee_data['medical_notes']}")
                    
                    with col_detail2:
                        st.markdown("**Event Information**")
                        st.markdown(f"**Ticket ID:** `{attendee_data['ticket_id']}`")
                        st.markdown(f"**Status:** {attendee_data['status'].replace('_', ' ').title()}")
                        st.markdown(f"**Registration Time:** {attendee_data['registration_time']}")
                        if attendee_data['checkin_time']:
                            st.markdown(f"**Check-in Time:** {attendee_data['checkin_time']}")
                        
                        # Team badges
                        if attendee_data['worship_team']:
                            st.markdown("**Team:** 🎵 Worship Team")
                        elif attendee_data['volunteer']:
                            st.markdown("**Team:** 🤝 Volunteer")
                        else:
                            st.markdown("**Team:** 👥 Attendee")
